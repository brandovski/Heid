...
export default function ProjecaoCalendario({ items, saldoPorDia, mes }: Props) {
  ...
  return (
    <div>
      <div className="bg-surface rounded-xl border border-brand-700/10 shadow-sm overflow-hidden mb-4">
        <div className="grid grid-cols-7 border-b border-brand-700/10">
          {weekDays.map((d) => (
            <div key={d} className="py-2 text-center text-xs font-semibold text-brand-700/40">{d}</div>
          ))}
        </div>
          ...
          {cells.map((cell, idx) => {
            ...
            return (
              <button
                ...
                className={`
                  relative flex flex-col items-center justify-start pt-1.5 pb-2 min-h-[52px] border-b border-r border-brand-700/5 transition-colors
                  ${!cell.isCurrentMonth ? "bg-brand-700/5 cursor-default" : "hover:bg-brand-50/50 cursor-pointer"}
                  ${isToday ? "bg-brand-50 border-brand-200" : ""}
                  ${isSelected ? "bg-brand-100 ring-1 ring-inset ring-brand-300" : ""}
                `}
              >
                <span className={`text-xs font-medium ${
                  !cell.isCurrentMonth ? "text-brand-700/30" :
                ...
                {cell.isCurrentMonth && dots.length > 0 && (
                  <div className="flex items-center gap-0.5 mt-1">
                    {dots}
                    {cellItems.length > 3 && (
                      <span className="text-[9px] text-brand-700/40 ml-0.5">+{cellItems.length - 3}</span>
                    )}
                  </div>
                )}
              </button>
      ...
      {/* Legenda */}
      <div className="flex items-center gap-3 text-xs text-brand-700/40 mb-4 px-1">
      ...
      {selectedDay && (
        <div className="bg-surface rounded-xl border border-brand-700/10 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 bg-surface border-b border-brand-700/10">
            <span className="text-xs font-semibold text-brand-700/70 uppercase tracking-wide">
          ...
          {selectedItems.length === 0 ? (
            <p className="px-4 py-6 text-sm text-brand-700/40 text-center">Nenhum lançamento neste dia.</p>
          ) : (
                ...
                return (
                  <div key={item.id} className={`flex items-center gap-3 px-4 py-3 ${isProjected ? "bg-brand-50/50" : ""}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-brand-700 truncate">{item.description}</p>
                      <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        ...
                        {item.contributorName && (
                          <span className="inline-block px-1.5 py-0.5 rounded text-[10px] font-medium bg-brand-700/10 text-brand-700/70">
                            {item.contributorName}
                          </span>
                        )}
                      </div>
      ...
      {!selectedDay && (
        <p className="text-xs text-brand-700/40 text-center py-4">
