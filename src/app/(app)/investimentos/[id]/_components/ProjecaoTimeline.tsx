...
function DayGroup({ date, items, saldo }: { date: string; items: ProjecaoItem[]; saldo: number }) {
  return (
    <div className="mb-4 bg-surface rounded-xl border border-brand-700/10 overflow-hidden shadow-sm">
      <div className="flex items-center justify-between px-4 py-2.5 bg-surface border-b border-brand-700/10">
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <Calendar size={40} className="text-brand-700/30 mb-3" />
        <p className="text-brand-700/50 text-sm">Nenhum lançamento previsto neste mês.</p>
        <p className="text-brand-700/40 text-xs mt-1">
