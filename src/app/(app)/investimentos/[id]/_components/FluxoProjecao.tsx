...
export default function FluxoProjecao({ saldoAtual, monthlyContribution, projectItems }: Props) {
  ...
  return (
    <div className="bg-surface rounded-xl border border-brand-700/10 overflow-hidden">
      <div className="px-5 py-4 border-b border-brand-700/10">
        <h2 className="text-sm font-semibold text-brand-700">Projeção de Saldo — 12 meses</h2>
        <p className="text-xs text-brand-700/40 mt-0.5">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-surface border-b border-brand-700/10">
              <th className="text-left px-4 py-2.5 text-xs font-semibold text-brand-700/50 uppercase tracking-wide">Mês</th>
              <th className="text-right px-4 py-2.5 text-xs font-semibold text-brand-700/50 uppercase tracking-wide">Aporte</th>
              <th className="text-right px-4 py-2.5 text-xs font-semibold text-brand-700/50 uppercase tracking-wide">Retiradas</th>
              <th className="text-right px-4 py-2.5 text-xs font-semibold text-brand-700/50 uppercase tracking-wide">Saldo Projetado</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-700/5">
            {projecao.map((row) => {
              const isCurrentMonth = row.mes === todayMes;
              return (
                <tr
                  key={row.mes}
                  className={isCurrentMonth ? "bg-brand-50" : "hover:bg-brand-700/5"}
                >
                  <td className="px-4 py-2.5 text-brand-700 font-medium whitespace-nowrap">
                    {row.label}
                    {isCurrentMonth && (
                  ...
                  <td
                    className={`px-4 py-2.5 text-right font-semibold whitespace-nowrap ${
                      row.saldo >= 0 ? "text-brand-700" : "text-rose-600"
                    }`}
                  >
